function Katelog (opts) {
  ...
  // 结合自身项目适配 由于使用MD插件渲染出来会出现两次目录，所以获取其中一个即可
  const $content = document.getElementsByClassName('v-show-content')[0] // 内容元素
    ...
    if (tree) {
      ...
      for (let i = 0; i < tree.length; i++) {
        if (isEqual(tree[i].parent, _parent)) {
          hasChild = true
          ul +=
            `<li title="${tree[i].name}"><div class="${option.linkClass} k-catelog-level-${
              tree[i].level
            }" data-target="${tree[i].id}">` +
            tree[i].name +
            '</div>'
      ...
      if (getDataset(c, 'target') === id) {
        ...
        if ($catelog.children[0].offsetHeight > $catelog.offsetHeight) {
          if (c.offsetTop > $catelog.offsetHeight / 2) {
            ...
            if (
              $catelog.children[0].offsetHeight - c.offsetTop - c.offsetHeight <
              $catelog.offsetHeight / 2
            ) {
              $catelog.children[0].style.marginTop =
                -$catelog.children[0].offsetHeight +
                $catelog.offsetHeight +
                'px'
            } else {
              $catelog.children[0].style.marginTop =
                $catelog.offsetHeight / 2 - c.offsetTop + 'px'
            }
          } else {
